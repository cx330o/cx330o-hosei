...
Groq API（LLaMA 3.3 70B）を活用した以下のAI機能を実装している。

### キャンパスAIチャットボット
RAG（Retrieval-Augmented Generation）ライクなアプローチで、キャンパス情報をコンテキストとしてLLMに渡し、学生の質問に回答する。学食、図書館、交通アクセスなどの情報に対応。

### 投稿自動要約
長い投稿を1〜2文に自動要約する。ユーザーの言語設定に応じて要約言語を切り替える。

### AI翻訳
4言語（日本語・英語・中国語・韓国語）間のリアルタイム翻訳。留学生と日本人学生のコミュニケーションを支援。

### コンテンツモデレーション
投稿内容の安全性をAIが自動判定。スパム、ハラスメント、不適切なコンテンツを検出。

### セマンティック検索
自然言語のクエリからキーワードを抽出し、関連する投稿を検索。従来のキーワード検索より柔軟な検索体験を提供。
